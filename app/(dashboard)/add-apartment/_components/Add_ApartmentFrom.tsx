...
interface FormData {
  ...
  day: string;
  month: string;
  availableTime: string; // datetime-local value
  ...
  inspectionStatus: string; // single select for simplicity (can be array later)
  keyExchangeInfo: string; // textarea or input — using string for now
  packageTracking: string; // textarea or input — using string for now
}
...
const AddApartment: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    // New required fields (adjust as needed — made most required)
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // Fixed: proper array format for amenities
      ...
      if (formData.availableTime) {
        formDataToSend.append(
          "availableFrom[time]",
          new Date(formData.availableTime).toISOString()
        );
      }
      ...
      formDataToSend.append("inspectionStatus", formData.inspectionStatus); // single value for now
      formDataToSend.append("keyExchangeInfo", formData.keyExchangeInfo);
      formDataToSend.append("packageTracking", formData.packageTracking);
      ...
      if (response.ok) {
        ...
        throw new Error(err || "Failed to create apartment listing");
      }
    } catch (error) {
      toast.error("Error creating apartment listing: " + (error as Error).message, {
        position: "top-right",
      });
    } finally {
      setIsSubmitting(false);
            ...
            <div className="">
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    {errors.inspectionStatus && (
                      <p className="text-red-500 text-sm mt-1">{errors.inspectionStatus}</p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-base font-medium text-[#000000] mb-2">Amenities</label>
                  <div className="grid grid-cols-3 gap-2">
                    {["Parking", "Lift", "Security", "Balcony", "Generator", "Air Conditioning"].map(
                      (amenity) => (
                        <label key={amenity} className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                ...
                <div>
                  ...
                  />
                  {errors.aboutListing && (
                    <p className="text-red-500 text-sm mt-1">{errors.aboutListing}</p>
                  )}
                </div>
              </div>
            </div>
